...
const DentistaPainel = ({ cpf, isAdmin, onClose }: { cpf: string; isAdmin: boolean; onClose: () => void }) => {
...
export const Dentistas = () => {
  ...
  return (
    <AsyncEstado loading={loading} error={error} vazio={!dentistas?.length} mensagemVazio="Nenhum dentista cadastrado.">
      <UserManagementPage<DentistaCompleto>
        ...
        renderDetails={(user, close) => (
          <DentistaPainel cpf={user.cpf} isAdmin={isAdmin} onClose={close} />
        )}
      />
    </AsyncEstado>
  );
