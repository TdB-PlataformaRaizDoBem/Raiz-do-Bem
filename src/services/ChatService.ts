...
export async function getChatHistory(
  ...
  const res = await safeFetch(`${CHAT_API_URL}/chat/history/${tel}?skip=${skip}&limit=${limit}`);
...
export async function sendMessage(
  ...
  const res = await safeFetch(`${CHAT_API_URL}/chat/send`, {
    method: "POST",
...
export async function getActiveConversations(): Promise<ConversationPreview[]> {
  try {
    const res = await safeFetch(`${CHAT_API_URL}/chat/conversations`);
    if (!res.ok) return [];
