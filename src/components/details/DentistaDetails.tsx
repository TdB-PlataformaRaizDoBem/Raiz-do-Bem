...
const SecaoDadosProfissionais = ({ data }: { data: DentistaViewModel }) => (
  <div className="bg-gray-50/50 p-6 rounded-2xl border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      <h4 className="text-xs font-black uppercase text-gray-400 mb-1">CRO</h4>
      <p className="text-darkgray font-medium text-lg">{data.croDentista}</p>
    </div>
...
const SecaoContato = ({ data }: { data: DentistaViewModel }) => (
  <div className="p-4 rounded-2xl border border-gray-100 bg-white shadow-sm">
    ...
    <p className="text-darkgray font-medium">Email: {data.email}</p>
    <p className="text-darkgray font-medium mt-1">Telefone: {data.telefone}</p>
  </div>
);

const SecaoEndereco = ({ data }: { data: DentistaViewModel }) => (
