...
import { getDentistasProximos } from "../../services/DentistaService";
...
export const DesignacaoDetails = ({ data, dentistaSelecionado, setDentistaSelecionado, onDesignar, onClose }: Props) => {
      ...
      <div className="flex flex-col max-h-[90vh] md:max-h-[85vh] w-full">
        <div className="flex-1 overflow-y-auto pb-24 p-1 pr-2 custom-scrollbar">
          ...
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="text-right">
              <span className="block text-xs font-black text-gray-400 uppercase tracking-widest">ID Beneficiário</span>
          ...
          <div className="space-y-6">
            <div className="flex gap-4 items-center bg-gray-50 p-4 rounded-xl border border-gray-100">
              ...
              <div>
                ...
                <p className="font-bold text-darkgray">
                  {data.endereco ? `${data.endereco.cidade} — ${data.endereco.estado}` : "Não informado"}
                </p>
              </div>
            </div>
            ...
            {data.pedido && (
              <div className="bg-lightgreen/5 p-6 rounded-2xl border border-lightgreen/20">
                <h4 className="text-xs font-black uppercase text-darkgreen tracking-wider mb-2">Pedido aprovado vinculado</h4>
                <p className="text-xs text-gray-500 font-mono">Protocolo #{data.pedido.id} · {data.pedido.dataPedido}</p>
                {/* era pedido.descricao_problema — campo do mock */}
                <p className="text-sm text-darkgray mt-3 italic leading-relaxed">"{data.pedido.descricaoProblema}"</p>
              </div>
            )}
            ...
            <div>
              ...
              {semDentistas && (
                <div className="p-4 rounded-xl bg-red-50 border border-red-200">
                  <p className="text-sm font-bold text-red-700">Nenhum dentista disponível</p>
                  <p className="text-xs text-red-500 mt-1 leading-relaxed">
                    Não há dentistas disponíveis em {data.endereco?.cidade ?? "sua cidade"} ou no estado {data.endereco?.estado ?? "informado"}.
                  </p>
                </div>
              )}
              ...
              {!loadingDentistas && dentistas.length > 0 && (
                <div className="flex flex-col gap-2">
                  {dentistas.map((d) => {
                    ...
                    const mesmaCidade = d.endereco?.cidade === data.endereco?.cidade || d.endereco?.estado === data.endereco?.estado;

                    return (
                      <button
                        key={d.id}
                        type="button"
                        onClick={() => setDentistaSelecionado(selecionado ? null : d)}
                      ...
                      >
                        <div className="flex justify-between items-start gap-2">
                          <div>
                            ...
                            <p className="font-bold text-darkgray text-sm">{d.nomeCompleto}</p>
                            {/* era d.cro — campo do mock */}
                            <p className="text-xs text-gray-500 mt-0.5">CRO: {d.croDentista}</p>
                            <p className="text-xs text-gray-400 mt-1">
                              {d.endereco ? `${d.endereco.cidade} — ${d.endereco.estado}` : "Localização não informada"}
                            </p>
                          </div>
                          <div className="flex flex-col items-end gap-1 shrink-0">
                            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${mesmaCidade ? "bg-lightgreen/20 text-darkgreen" : "bg-amber/10 text-amber"}`}>
                              {mesmaCidade ? "Mesma cidade" : "Mesmo estado"}
                            </span>
                            {selecionado && <span className="text-[10px] font-black text-darkgreen uppercase tracking-wide">Selecionado</span>}
                          </div>
                        </div>
                      </button>
                    );
        ...
        <UserActions>
          <div className="flex flex-wrap md:flex-nowrap gap-3 justify-end w-full">
            <Button variant="secondary" onClick={onClose} className="flex-1 md:flex-none">Fechar</Button>
            <Button
              variant="primary"
              ...
              onClick={() => dentistaSelecionado && onDesignar(data, dentistaSelecionado)}
            >
              {/* era dentistaSelecionado.nome — campo do mock */}
              {dentistaSelecionado ? `Designar ${dentistaSelecionado.nomeCompleto.split(" ")[0]}` : "Selecione um dentista"}
            </Button>
          </div>
        </UserActions>
      </div>
